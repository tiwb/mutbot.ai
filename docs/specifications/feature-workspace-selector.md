...
### 手动测试
- [ ] localhost 无 hash → 显示工作区选择器（两栏布局）
- [ ] localhost `#my-project` → 直接进入工作区
- [ ] localhost 选择工作区 → hash 更新 + 加载
- [ ] localhost 新建工作区 → 目录选择器 → 创建 → 跳转
- [ ] localhost 新建同名工作区 → 提示重命名
- [ ] localhost 工作区 > 5 个时显示 "More..."（蓝色）→ 弹出搜索对话框
- [ ] localhost 搜索对话框 → 输入关键词过滤 → 选择 → 进入工作区
- [ ] mutbot.ai 页面加载 → "Run MutBot" 区域在 Start 之前可见
- [ ] mutbot.ai Run MutBot 标签切换：Shell / uv / pip
- [ ] mutbot.ai Shell 标签自动检测平台（macOS 显示 curl，Windows 显示 irm）
- [ ] mutbot.ai 连接成功 → 工作区列表（按最近访问排序）+ New Workspace 出现
- [ ] mutbot.ai 连接失败（桌面）→ 引导提示（不含 "install" 字样）
- [ ] mutbot.ai 连接失败（移动端）→ PC 安装 + 远程操控提示
- [ ] mutbot.ai 工作区 > 5 个时显示 "More..."（蓝色）→ 弹出搜索对话框
- [ ] mutbot.ai 选择工作区 → 重定向到 localhost
- [ ] mutbot.ai `#my-project` → 连接成功后自动重定向
- [ ] mutbot.ai 新建工作区 → 目录选择器（CORS）→ 创建 → 重定向
- [ ] mutbot.ai 搜索对话框样式与页面一致（字体、颜色）
- [ ] 响应式：移动端左右变上下
- [ ] Chrome / Firefox / Safari 各浏览器验证
